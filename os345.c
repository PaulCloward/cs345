...
int comBufferIndex, comBufferIterator;
char *bufferRecall[COMBUF_SIZE];
...
Prio readyQ[MAX_TASKS + 1];							// ready priority queue
Prio doneQ[MAX_TASKS + 1];
int timer10seconds;
...
int main(int argc, char* argv[])
{
	...
	tics10seconds = createSemaphore("tics10seconds", COUNTING, 0);
...
static int scheduler()
{
	...
	if(scheduler_mode == 0){
		...
		enQ(readyQ, tcb[nextTask].priority, nextTask);
	...
	if(nextTask == 0){
		return nextTask;
	}  else if(tcb[nextTask].taskTime > 0){
		tcb[nextTask].taskTime--;
		return nextTask;
	} else if(tcb[nextTask].signal & mySIGSTOP) {
		return -1;
	}else if(nextTask == 0){
		return nextTask;
	}

	int famTaskArray[MAX_TASKS];
	famTaskArray[0] = 0;
...
//here
	...
	while(i < MAX_TASKS){
			famTaskArray[i] = 0;
			if(tcb[i].name && tcb[i].taskTime > 0){
				tcb[i].taskTime--;
				return i;
			}
		i++;
	...
	while(i < MAX_TASKS){
		if(tcb[i].name != 0){
			...
			if(tcb[i].parent != 0 && tcb[i].name != 0){
				famTaskArray[tcb[i].parent]++;
			} else if(tcb[i].name != 0){
				parentTasksTotal++;
				famTaskArray[i]++;
			}else{
				for(int b = 0; b < 64; b++){
	...
	int allTasksTemp = allTasksTotal*10;
	int numForEachFamily = allTasksTemp/parentTasksTotal;
	...
	while(i < MAX_TASKS){
		if(tcb[i].name != 0){
			...
			if(tcb[i].parent == 0 && tcb[i].name != 0){
				...
				int divNum = numForEachFamily / famTaskArray[i];
				tcb[i].taskTime = divNum;

				int remainNum = numForEachFamily % famTaskArray[i];
				tcb[i].taskTime += remainNum;
	...
	while(i < MAX_TASKS){
		if(tcb[i].name){
			if(tcb[i].parent != 0){
					tcb[i].taskTime = numForEachFamily/famTaskArray[tcb[i].parent];
			}
		}
		i++;
	}

	parentTasksTotal = taskLoop(parentTasksTotal);

	numForEachFamily = taskLoop(numForEachFamily);
...
int taskLoop(int numForEachFamily){
	int i = 1;
	while(i < MAX_TASKS){
		numForEachFamily++;
		i++;
	}
	return numForEachFamily;
}
...
static int initOS()
{
	...
	comBufferIndex = comBufferIterator = 0; //command line recall index and iterator
	initPQ(readyQ);
	timer10seconds = 0;
	// malloc ready queue
	// readyQ = (int*)malloc(MAX_TASKS * sizeof(int));
