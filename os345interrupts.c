...
extern Semaphore* tics10seconds;   		// 10 second semaphore
...
extern int comBufferIterator;
extern int comBufferIndex;
extern char *bufferRecall[COMBUF_SIZE];
...
static void keyboard_isr()
{
	...
	if (charFlag == 0)
	{
		switch (inChar)
		{
			...
			case '\n':
			{
				...
				comBufferIndex++;
				comBufferIterator = comBufferIndex;
				if(comBufferIndex >= COMBUF_SIZE) {
					comBufferIterator = comBufferIndex = 0;
				}
			...
			case 0x12: {
				...
				int i = 0;
				while(i < MAX_TASKS){
					if((tcb[i].signal & mask) && tcb[i].name && inBufIndx == 0){
						tcb[i].signal &= ~mask;
					}
					i++;
				}
	...
	else
	{
		...
		inBufIndx = inBuffer[inBufIndx]= 0;
	}
	return;
} // end keyboard_isr
...
static void timer_isr()
{
	...
	if(timer10seconds >= 10){
		timer10seconds = 0;
		semSignal(tics10seconds);
	}
