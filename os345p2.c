...
extern Semaphore* tics10seconds;
...
// ?? 3) If blocked, indicate which semaphore
	...
	printf("\n");
	printQueue(readyQ);
	Semaphore* semTemp = semaphoreList;

	while(semTemp){
		if((argc == 1) || match(argv[1], semTemp->name)){
			printQueue(semTemp->pq);
		}
		semTemp = (Semaphore*)semTemp->semLink;
	}
...
int tic10Task(int argc, char* argv[]){
	char svtime[64];
	while(1){
		SWAP;
		SEM_WAIT(tics10seconds);
		printf("\ntics10seconds, Current task = %d, Time = %s", curTask, myTime(svtime));
	}
	return 0;
}
