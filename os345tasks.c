					...
					char* argv[])				// task argument pointers
{
	...
	for (tid = 0; tid < MAX_TASKS; tid++)
	{
		if (tcb[tid].name == 0)
		{
			...
			char **newArgv = (char**)malloc(sizeof(char*) * argc);//might need to be global or static
			for(int i = 0; i < argc; i++) {
				newArgv[i] = (char *)malloc(strlen(argv[i])+1);//malloc the size of the string + 1 for null terminator
...
static void exitTask(int taskId)
{
	...
	while(i < MAX_TASKS){
		if(tcb[i].parent == taskId){
			...
			if(newParent == 0 && tcb[i].parent == taskId){
				newParent = i;
			}
		}
		i++;
