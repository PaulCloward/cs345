...
int P1_shellTask(int argc, char* argv[])
{
	...
	while (1)
	{
...

		{
			...
			// init arguments
			char *memoryIterator = (char*)malloc(sizeof(char*) * 9 * MAX_ARGS);

			sp = inBuffer;

			newArgc = 1;
			myArgv[0] = memoryIterator;
			bool inQuotes = 0;
			...
			while(*sp != 0) {
				...
				if(*sp == ' ' && !inQuotes) {
					*memoryIterator++ = 0;//Assign zero then increment iterator
					sp++;
					myArgv[i++] = memoryIterator;
					newArgc++;

				} else {

					*memoryIterator++ = *sp++;
					//printf("\nCharacter: %s", memoryIterator-1);
				}
			}
	...
	//			*sp++ = 0;
		...
		// look for command
		// if(runInBackground) {
		// 	printf("We have an & at the end, run in background");
		// 	runInBackground = 0;
		// }
		...
		for (found = i = 0; i < NUM_COMMANDS; i++)
		{
				 ...
				 !strcmp(newArgv[0], commands[i]->shortcut))
			{
				...
				if(runInBackground) {
					...
					//createTask(commands[i]->command,(*commands[i]->func)(newArgc, newArgv), 0, newArgc, newArgv);
...
int P1_add(int argc, char* argv[])
{
	...
	for(i = 1; i < argc; i++) {
		if(argv[i][0] == '%') {
			total += (int)strtol(argv[i] + 1, NULL, 2);
		} else {
...
Command** P1_init()
{
	...
	commands[i++] = newCommand("add", "+", P1_add, "Adds to integers together");
