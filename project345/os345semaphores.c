...
//
void semSignal(Semaphore* s)//increases resources
{
	int i;
	...
	if (s->type == 0)
	{
		...
		if (s->state == 0)//if its used up, free it
		{
			int i = deQ(s->pq, -1);
			if(i > -1){
				enQ(readyQ, tcb[i].priority, i);
			...
			//enQ(readyQ, tcb[i].priority, deQ(s->pq, i))
